...
public class ExpressionSpecificationTester
{
	...
	public void MoreThan10_OrOp_LessThan5()
	{
		...
		Assert.That(subject, Is.Not.SatisfiedBy(7));
		Assert.That(subject, Iz.SatisfiedBy(3).Or(13));
	}
	...
	private readonly Specification<ComplexType> _fooLengthOf2 = new ExpressionSpecification<ComplexType>(c => c.Foo.Length == 2);

	[Test]
	public void ComplexComposition_PredicateUsage_FoundMatchingElements()
	{
		var data = new List<ComplexType>(new ComplexContainer());
		Assert.That(data.Find(_fooLengthOf2.IsSatisfiedBy), Has.Property(nameof(ComplexType.Bar)).EqualTo(2));
